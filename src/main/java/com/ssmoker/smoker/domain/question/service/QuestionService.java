...
public class QuestionService {
    ...
    public QuestionResponse.QuestionListResponse getQuestionList(Integer page) {
        if (page == null || page < 1) {
            throw new SmokerClientException(ErrorStatus.QUESTION_BAD_REQUEST);
        }
        Pageable pageable
    ...
    public QuestionResponse.QuestionDetailResponse getQuestionDetail(Long questionId) {
        Question question = questionRepository.findById(questionId).orElseThrow(() -> new SmokerClientException(ErrorStatus.QUESTION_NOT_FOUND));
