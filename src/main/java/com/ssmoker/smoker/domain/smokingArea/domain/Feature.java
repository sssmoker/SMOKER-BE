...
public class Feature {
    ...
    private Boolean hasAirPurifier = false; // 공기 청정 기능

    @Column(nullable = false)
    private Boolean hasAirConditioning = false; // 냉난방 기능

    @Column(nullable = false)
    private Boolean hasChair = false; // 의자 제공

    @Column(nullable = false)
    private Boolean hasTrashBin = false; // 쓰레기통

    @Column(nullable = false)
    private Boolean hasVentilationSystem = false; // 환기 시스템

    @Column(nullable = false)
    private Boolean isAccessible = false; // 휠체어 진입 가능 여부

    @Column(nullable = false)
    private Boolean hasCCTV = false; // CCTV 설치 여부

    @Column(nullable = false)
    private Boolean hasEmergencyButton = false; // 비상 버튼

    @Column(nullable = false)
    private Boolean hasVoiceGuidance = false; // 음성 안내 시스템

    @Column(nullable = false)
    private Boolean hasFireExtinguisher = false; // 소화기 비치 여부

    @Column(nullable = false)
    private Boolean isRegularlyCleaned = false; // 정기적인 청소 여부

    @Column(nullable = false)
    private Boolean hasCigaretteDisposal = false; // 담배꽁초 처리함 제공 여부

    @Column(nullable = false)
    private Boolean hasSunshade = false; // 햇빛 차단 시설

    @Column(nullable = false)
    private Boolean hasRainProtection = false; // 비바람 차단 시설
    ...
    public static Feature makeEmptyFeature() {
        return new Feature(false,false,false,false,false,false,false,false,false,false,false,false,false,false);
    }
}
